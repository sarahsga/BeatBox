...
class BeatBoxSerial {
	...
	public void buildGui() {
		...
		buttonBox.setBackground(Color.GREEN);
		...
		serializeIt.addActionListener(new MyClearListener());
		buttonBox.add(clear);
		...
		public void actionPerformed(ActionEvent ev) {
			try {
				...
				for ( int i = 0 ; i < 16 ; i ++ ) {
					for ( int j = 0 ; j < 16 ; j++ ) {
						JCheckBox jc = (JCheckBox) checkboxList.get(j + (16*i));
						oos.writeBoolean(jc.isSelected());
					}
				}
				oos.close();
	...
	public class MyRestoreListener implements ActionListener {
			...
			try {
				...
				if ( isChosen == true ) {
					for ( int i = 0 ; i < 16 ; i ++ ) {
						for ( int j = 0 ; j < 16 ; j++ ) {
							JCheckBox jc = (JCheckBox) checkboxList.get(j + (16*i));
							jc.setSelected(ois.readBoolean());
						}
					}
					ois.close();
				}
			} catch(Exception ex) { System.out.println("Invalid file... error"); }		
		}
		...
		private boolean loadFile() {
			try {
				...
				if (result == fileChooser2.APPROVE_OPTION) {
					chosenFile = fileChooser2.getSelectedFile();
					filePath = chosenFile.getPath();
					...
					return false;
			} catch(Exception ex ) { System.out.println("loading error ... inside load"); return false;}
		}
	}
